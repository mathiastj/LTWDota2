...
function BuildingHelper:AddBuilding(keys)
  ...
  player.mgd2 = CreateUnitByName("npc_ground_prop", OutOfWorldVector, false, nil, nil, builder:GetTeam())
  ...
  CustomGameEventManager:Send_ServerToPlayer(player, "building_helper_enable", {["state"] = "active", ["size"] = size, ["entIndex"] = player.mgd2:GetEntityIndex(), ["MaxScale"] = 0.6} )
end
...
function BuildingHelper:InitializeBuildingEntity( keys )
  ...
  if callbacks.onConstructionStarted ~= nil then
    ...
    -- Remove all move to abilities once construction is started
